class ContentPublisher
  ...
  def add_to_group(membership, message)
    ...
    if (membership.messages.any? && membership.messages.order(:sent_at).last.sender_id == message.sender_id)
      HollerbackApp::BaseApp.logger.info "sajjad: grouping"
      ...
      if (membership.message_groups.any?)
        ...
        if (groups.any?)
          ...
          HollerbackApp::BaseApp.logger.info "sajjad: using an existing group with id #{group.id}"
          group << message
        ...
        if(message.sent_at - last_message.sent_at <= 60 && last_message.sender_id == message.sender_id)
          HollerbackApp::BaseApp.logger.info "sajjad: creating a new message group: lid: #{last_message.sent_at} cid: #{message.sender_id}"
      ...
      HollerbackApp::BaseApp.logger.info "sajjad: not grouping"
    end
